...
module Alice2
    ...
    class Build
        ...
        state_machine :state, :initial => :waiting_for_new_build do
            # transition that hijack the normal ones to test 
            ...
            # IDEA:  if put each :do input as a method list, then it would be very useful to 
            # allow user to validate input, with limited 
            after_transition :waiting_for_new_build => :sync_workspace,
                         :do => [:basic_check_workspace]
            
            
            before_transition :sync_workspace => :build_cpp, 
                                  :do => [:verify_prerequisite_for_build_cpp]
            
            after_transition :sync_workspace => :build_cpp, 
                                          :do => [ :verify_prerequisite]
            ...
            # event requirement
            # -----------------------------
            event :to_start_nightly_build do 
                transition :waiting_for_new_build => :sync_workspace 
            end
            
            event :to_build_cpp do 
                transition :sync_workspace => :build_cpp
            end
        ...
        def clean_workspace
            ...
            # it looks like that the 'clean_workspace is hard coded, in this process, but it can be viewed as 
            # try to incorporate the actions strategies patterns, default to make clean if makefile present 
            # also can be served as a contract/interface
            
            # Note: there may be no need to refactor because very rare a new Builder will be created.
            ab = Alice2::Builder.new("D:/zjc/wr/sandbox")  #  enumerated options can be clean_by(:makefile) 
            ab.clean_workspace
            
            self.send :to_start_nightly_build
            self.send :to_build_cpp
            # IDEA: is it poss
        ...
        def issue_next_op_from(event)
            # TODO, debug logger here, important!
            self.send $EVENTS_DRIVEN[event.to_s] 
        end
        
        def verify_prerequisite_for_build_cpp
